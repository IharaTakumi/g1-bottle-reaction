...
SCRIPT = ROOT / 'scripts/g1-pc2-close-slam-once.py'


def load_builder():
    spec = importlib.util.spec_from_file_location(
        'build_close_once', ROOT / 'scripts/build-pc2-close-slam-once.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_close_client_is_python38_and_has_one_official_rpc_only():
    ...
    assert rpc_calls[0].args[0].value == 1901
    ...
    for forbidden in ('1801', '1802', '1804', 'LowCmd', 'Motion', 'Navigation', 'arm_sdk', 'armsdk'):
        assert forbidden not in source


def test_stdin_close_client_is_standalone_and_preserves_guards():
    ...
    assert source.count('client._Call(1901,') == 1
